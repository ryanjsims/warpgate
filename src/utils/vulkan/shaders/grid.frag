...
float computeDepth(vec3 pos) {
    vec4 clip_space_pos = ubo.projectionMatrix * ubo.viewMatrix * vec4(pos.xyz, 1.0);
...
float computeLinearDepth(vec3 pos) {
    ...
    float linearDepth = (2.0 * clipPlanes.near * clipPlanes.far) / (clipPlanes.far + clipPlanes.near - clip_space_depth * (clipPlanes.far - clipPlanes.near)); // get linear value between 0.01 and 100
    return linearDepth / clipPlanes.far; // normalize
}
